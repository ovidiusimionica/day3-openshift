...
Vagrant.configure("2") do |config|
  ...
  (1..2).each do |i|
    config.vm.define "node0#{i}" do |node|
      ...
      node.vm.hostname = "node0#{i}.#{DOMAIN}"

      if "#{i}" == "1"
        node.hostmanager.aliases = "lb.#{DOMAIN}"
      end
    end
  end
  ...
  config.vm.define "master", primary: true do |node|
    ...
    node.hostmanager.aliases = "etcd.#{DOMAIN}"
  
    node.hostmanager.aliases = "nfs.#{DOMAIN}"
    # 
    ...
    node.vm.provision "shell", inline: <<-SHELL
      /vagrant/master.sh #{OPENSHIFT_RELEASE} #{OPENSHIFT_ANSIBLE_BRANCH} #{NETWORK_BASE} #{DOMAIN}
    SHELL
